...
tags: ["query:sr"]
score: 10.0
evidence: 对 PySR 和 PhySO 等最先进符号回归框架的系统比较研究
...
## 摘要
本研究探讨了符号回归（SR）作为黑盒机器学习的一种可解释替代方案，在斯隆数字巡天第17次数据发布（SDSS DR17）中用于恒星、星系和类星体的分类。我们对四种最先进的符号回归框架进行了系统的比较研究：PySR、基于最小描述长度（MDL）选择的穷举符号回归（ESR）、利用深度强化学习的物理符号优化（PhySO）以及多视图符号回归（MvSR）。通过在具有代表性的训练子集上推导出紧凑的解析函数（复杂度 ≤ 10），并随后在10万个经过光谱确认的天体上通过五折分层交叉验证协议进行评估，我们将光谱红移（z）映射为连续的分类评分。结果表明，这些低复杂度的表达式实现了极高的预测可靠性，其中 MvSR 的 Cohen's Kappa 系数达到 0.8948，而 PhySO 表现出卓越的参数稳定性（σ < 0.002）。我们证明，这些模型不仅达到了传统基准模型的性能，还为区分银河系内与河外星系群体的天体物理边界提供了透明且数学简洁的表征。
