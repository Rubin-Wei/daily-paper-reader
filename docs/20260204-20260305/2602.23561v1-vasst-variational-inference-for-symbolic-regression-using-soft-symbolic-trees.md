...
motivation: 现有符号回归方法多依赖启发式搜索且缺乏不确定性量化，难以在庞大的多模态组合空间中进行高效探索。
method: 提出 VaSST 框架，通过软符号树对表达式结构进行连续松弛，将离散组合搜索转化为基于梯度的变分推理优化问题。
result: 在模拟实验和 Feynman 符号回归数据库的测试中，VaSST 在结构恢复准确度和预测精度方面均达到了最先进水平。
conclusion: VaSST 证明了结合连续松弛与变分推理可以有效解决符号回归中的搜索效率与概率建模难题。
---
