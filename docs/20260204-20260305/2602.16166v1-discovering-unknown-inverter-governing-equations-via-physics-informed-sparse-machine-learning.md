...
motivation: 现有方法在识别逆变器未建模非线性特性时，难以兼顾物理一致性与模型的可分析性。
method: 提出PISML框架，通过稀疏符号骨架捕捉主干模型，利用神经残差补偿非线性，并结合雅可比正则化训练与符号回归实现显式建模。
result: 在硬件在环实验中，该方法将识别误差降低了340倍以上，并成功将重型神经网络压缩为紧凑的显式数学方程。
conclusion: 该框架实现了从测量数据到显式控制方程的可靠映射，为结构不可达设备的电力系统稳定性分析提供了统一的解决路径。
---

## 摘要
从外部测量中发现并网逆变器的未知控制方程，对于分析现代逆变器密集型电力系统具有重要意义。然而，现有方法难以在识别未建模非线性与保持物理一致性之间取得平衡。为此，本文提出了一种物理信息稀疏机器学习（PISML）框架。该架构集成了一个用于捕捉主导模型骨架的稀疏符号主干，以及一个用于补偿复杂非线性控制逻辑的神经残差分支。同时，引入了一种雅可比正则化的物理信息训练机制，以强制执行包括大/小尺度行为在内的多尺度一致性。此外，通过对神经残差分支进行符号回归，PISML 实现了从黑盒数据到显式控制方程的可处理映射。在高保真硬件在环平台上的实验结果证明了该框架的优越性能。它不仅通过比基准方法降低 340 倍以上的误差实现了高分辨率识别，还实现了将庞大的神经网络压缩为紧凑的显式形式。这为严谨的稳定性分析恢复了解析可处理性，并将计算复杂度降低了几个数量级。它还提供了一种统一的途径，将结构不可访问的设备转换为显式数学模型，从而能够对具有未知逆变器控制方程的电力系统进行稳定性分析。
