...
tags: ["query:cd"]
score: 8.0
evidence: 将KV缓存压缩视为蒸馏以支持长上下文推理
tldr: 针对长文本LLM推理中KV缓存带来的内存压力，本文提出KVSculpt。不同于传统的剔除或合并策略，KVSculpt将压缩视为蒸馏过程，在连续嵌入空间中优化一组不受限的KV对，以模拟原始注意力行为。通过L-BFGS优化Key并用最小二乘法求解Value，结合自适应预算分配机制，在保持推理效率的同时显著提升了压缩精度。
source: arxiv
...
motivation: 现有的KV缓存压缩方法受限于原始缓存项的离散选择或简单合并，无法充分利用连续空间进行更高效的注意力行为模拟。
method: 将KV压缩建模为蒸馏问题，在连续空间交替使用L-BFGS优化Key和最小二乘法求解Value，并引入基于试点运行的跨层及跨头自适应预算分配。
result: 在Qwen2.5-1.5B模型上，KVSculpt在多种压缩率下将KL散度降低了3.5-4.1倍，自适应分配机制进一步带来了1.3倍的性能提升。
conclusion: 通过在连续空间优化KV对并实施细粒度的预算分配，可以显著超越基于选择或合并的传统KV缓存压缩方案。
---
