...
tldr: 本文针对大型视觉语言模型（LVLM）因处理高分辨率输入产生海量视觉标记而导致的计算开销巨大问题，全面综述了加速推理的前沿技术。文章系统性地将优化框架归纳为视觉标记压缩、内存管理与服务、高效架构设计及高级解码策略四个维度，并探讨了现有挑战与未来研究方向，旨在推动高效多模态系统的部署。
source: arxiv
...
motivation: LVLM在处理高分辨率图像时产生的海量视觉标记导致计算需求呈二次方增长，严重限制了其在大规模场景下的部署效率。
method: 本文提出了一种系统性的分类法，从视觉标记压缩、内存管理、架构优化和解码策略四个核心维度对现有的推理加速技术进行分类综述。
result: 通过对各类优化技术的深入分析，揭示了不同方案在降低计算复杂度、优化内存占用及提升推理速度方面的有效性。
conclusion: 论文总结了当前LVLM推理优化的局限性，并为未来开发更具扩展性和高效性的多模态系统指明了关键的研究路径。
---
