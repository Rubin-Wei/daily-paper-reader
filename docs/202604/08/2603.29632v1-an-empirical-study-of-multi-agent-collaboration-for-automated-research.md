...
tags: ["query:am"]
score: 8.0
evidence: 具有显式全局记忆的自动化研究多代理范式
tldr: 本研究针对自动化科研中多智能体系统（MAS）的协作框架进行了实证研究。通过在受控的机器学习优化测试平台上对比单智能体、子智能体（并行探索）和智能体团队（专家移交）三种架构，揭示了系统稳定性与理论深度之间的权衡。研究发现，子智能体模式适合快速广度搜索，而团队模式在长时计算下能实现深层架构重构，为未来动态路由的科研智能体设计提供了指导。
source: arxiv
...
result: 实验表明子智能体架构具有高韧性和高吞吐量，适合浅层优化；而智能体团队架构虽更脆弱，但在充足预算下能实现深层理论对齐和复杂重构。
conclusion: 未来自动化科研系统应采用动态路由架构，根据任务复杂度和实时需求灵活调整多智能体协作模式。
---

## 摘要
随着人工智能智能体（AI agents）的发展，研究界正迅速从单一的大语言模型（LLMs）转向多智能体系统（MAS），以克服自动化研究中的认知瓶颈。然而，这些自主智能体的最优多智能体协调框架在很大程度上仍未得到探索。在本文中，我们提出了一项系统的实证研究，调查了不同多智能体结构在自动化机器学习优化中的比较效能。利用一个配备了 Git 工作树隔离和显式全局内存的、严格受控的基于执行的测试平台，我们将单智能体基准与两种多智能体范式进行了对比：子智能体架构（具有事后整合的并行探索）和智能体团队架构（具有执行前交接的专家协作）。通过在严格固定的计算时间预算下评估这些系统，我们的研究结果揭示了运行稳定性与理论深思熟虑之间的基本权衡。子智能体模式表现为一个具有高韧性、高吞吐量的搜索引擎，最适合在严格的时间限制下进行广泛且浅层的优化。相反，由于多作者代码生成，智能体团队拓扑结构表现出更高的运行脆弱性，但在计算预算充足的情况下，它能实现复杂架构重构所需的深度理论对齐。这些实证见解为设计未来的自动研究系统提供了可操作的指导方针，提倡采用能够根据实时任务复杂度调整其协作结构的动态路由架构。
